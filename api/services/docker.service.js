...
// ES: Servicio principal de escaneo (descubrimiento de red, Nmap profundo y comprobaciones Hydra).

// EN: Services that can be tested by Hydra credential checks.
// ES: Servicios que pueden ser verificados por Hydra.
const hydraTargets = {
    "22": "ssh",
    "21": "ftp",
    "23": "telnet",
...
// ES: Imagen del escaner y red Docker opcional para laboratorio.
const scannerDockerImage = process.env.KALI_CONTAINER || "kali-redteam";
const scannerDockerNetwork = (process.env.SCANNER_DOCKER_NETWORK || "").trim();
...
// ES: Patrones de salida para detectar controles defensivos en servicios objetivo.
 ...
 * ES: Parsear y acotar valores enteros de variables de entorno.
 */
 ...
 * ES: Parsear valores booleanos de entorno con fallback seguro.
 */
 ...
 * ES: Parsear variables de entorno tipo enum.
 */
 ...
 * ES: Construir politica efectiva de Nmap desde variables de entorno y perfil.
 */
 ...
 * ES: Construir la politica efectiva de ejecucion de Hydra desde variables de entorno.
 */
 ...
 * ES: Inferir senales de lockout/rate-limit desde la salida de Hydra.
 */
 ...
 * ES: Extraer un resumen compacto y util para la UI.
 */
 ...
 * ES: Calcular direccion de red y broadcast desde un CIDR.
 */
 ...
 * ES: Verificar si una razon de descubrimiento suele ser ambigua/ruidosa.
 */
 ...
 * ES: Detectar interfaces IPv4 locales y derivar su subred CIDR.
 */
    ...
    for (const [name, addrs] of Object.entries(interfaces)) {
        for (const addr of addrs) {
            if (addr.family === "IPv4" && !addr.internal) {
                ...
                const cidr = maskParts.reduce((acc, octet) => acc + octet.toString(2).split("1").length - 1, 0);

                // EN: Compute network address (IP AND netmask).
                // ES: Calcular direccion de red (IP AND mascara).
 ...
 * ES: Descubrir hosts activos en una subred usando salida XML de Nmap.
 */
...
export async function discoverHosts(subnet) {
    ...
    const discoveryNetworkFlags = scannerDockerNetwork
        ? `--network=${scannerDockerNetwork}`
        : "--net=host --cap-add=NET_RAW --cap-add=NET_ADMIN";
    const cmd = `docker run --rm --memory="512m" --cpus="1" ${discoveryNetworkFlags} ${scannerDockerImage} nmap ${nmapArgs} ${subnet}`;
    ...
    try {
        ...
        let devices = rawDevices.filter(device => {
            ...
            // ES: Excluir red/broadcast para reducir ruido evidente.
        ...
        // ES: En redes tethering/NAT (ej. hotspot), el gateway puede responder
        // ES: con muchos reset que parecen hosts "up" falsos.
 ...
 * ES: Ejecutar escaneo profundo del host y comprobaciones Hydra opcionales.
 * ES: Incluye deteccion de servicios/version, scripts, OS y traceroute.
 */
export async function runDeepScan(target) {
    const scanPolicy = getScanPolicy();

    // EN: Two-phase strategy: fast probe first, rich detail only on open ports.
    // ES: Estrategia en dos fases: sondeo rapido y detalle solo en puertos abiertos.
    const probeArgs = [
        "-Pn",
        "-n",
        "-T4",
    ...
    const runtimeNetworkFlag = scannerDockerNetwork ? ` --network=${scannerDockerNetwork}` : "";
    const probeCommand = `docker run --rm --memory="1g" --cpus="2"${runtimeNetworkFlag} ${scannerDockerImage} nmap ${probeArgs} ${target}`;
    ...
    const detailCommand = `docker run --rm --memory="1g" --cpus="2"${runtimeNetworkFlag} ${scannerDockerImage} nmap ${detailArgs} ${target}`;
    ...
    // EN/ES: Address and hostname parsing / Parseo de direccion y hostname.
    ...
    // EN/ES: OS detection candidates / Candidatos de deteccion de sistema operativo.
    ...
    // EN/ES: Ports with normalized service details / Puertos con detalle normalizado.
    ...
    // EN/ES: Host scripts (post-scan) / Scripts de host (post-scan).
    const hostScripts = host.hostscript?.[0]?.script?.map(s => ({
        id: s.$.id,
        output: s.$.output || s._ || ""
    })) || [];

    // EN/ES: Extract coarse vulnerability signals from script output.
    // ES/EN: Extraer senales basicas de vulnerabilidad desde scripts.
    ...
    // EN/ES: Run Hydra only on supported open auth services.
    // ES/EN: Ejecutar Hydra solo en servicios de autenticacion abiertos y soportados.
    ...
    if (!hydraPolicy.enabled) {
        ...
        for (const p of hydraQueue) {
            ...
            // ES: Generar solo los primeros N pares usuario:password para limitar intentos.
            ...
            const hydraDockerCmd = `docker run --rm --memory="512m" --cpus="1"${runtimeNetworkFlag} ${scannerDockerImage} sh -lc '${prepPairsCmd} && ${hydraCmd}'`;
    ...
    // EN/ES: Build compact network summary for frontend cards.
    // ES/EN: Construir resumen compacto de red para tarjetas del frontend.
    const networkInfo = {
        host_ip: address,
        hostname: hostname,
