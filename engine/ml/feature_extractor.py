...
def extract_endpoint_features(events: Iterable[Mapping[str, Any]], window_seconds: int = 10) -> dict[str, float | int | bool]:
    ...
    return {
        "files_modified_per_second": action_counter.get("modified", 0) / float(window_seconds),
        "files_renamed_per_second": action_counter.get("renamed", 0) / float(window_seconds),
        "files_deleted_per_second": action_counter.get("deleted", 0) / float(window_seconds),
        ...
        "api_crypto_calls_per_second": sum(crypto_calls) / float(window_seconds),
...
def extract_correlation_features(
    ...
    *,
    ...
    mass_file_write = float(endpoint_features.get("files_modified_per_second", 0.0)) > 50.0
    mass_encryption = (
        float(endpoint_features.get("entropy_avg_modified_files", 0.0)) > 7.5
        and float(endpoint_features.get("entropy_delta", 0.0)) > 1.0
    )
    new_unknown_process = (
        endpoint_activity
