...
class AnalysisEngine:
    ...
    def __init__(self, event_callback: Callable[[dict[str, Any]], None] | None = None) -> None:
        ...
        correlation_rules_path = os.getenv("CORRELATION_RULES_PATH", "./engine/correlation_rules.json")
        ...
        self._endpoint_events: deque[dict[str, Any]] = deque(maxlen=500)
    ...
    def get_risk_score(self) -> dict[str, Any]:
        ...
        with self._lock:
            return asdict(self._risk_state)
    ...
    def _emit_endpoint_findings(
        ...
        if action == "modified" and float(endpoint_features.get("files_modified_per_second", 0.0)) > 50.0:
            self._findings.append(
                Finding(
                    ...
                    status="open",
                    created_at=datetime.now(timezone.utc).isoformat(),
                    details={"files_modified_per_second": endpoint_features.get("files_modified_per_second", 0.0)},
                )
            )
    ...
    def _build_top_reasons(
        ...
        if float(endpoint_features.get("files_modified_per_second", 0.0)) > 50.0:
            reasons.append("Se detecto modificacion masiva de archivos en pocos segundos.")
        if float(endpoint_features.get("entropy_avg_modified_files", 0.0)) > 7.5:
